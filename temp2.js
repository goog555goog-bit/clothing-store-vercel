global.sessionStorage = { getItem: () => null, setItem: () => {} }; global.localStorage = { getItem: () => null, setItem: () => {} }; global.document = { querySelectorAll: () => [], getElementById: () => null, body: { classList: { toggle: () => {} } }, addEventListener: () => {} }; global.window = { location: { hostname: '' }, onload: null }; global.API = { hasRole: () => false, hasPermission: () => false }; 
    ...
    function renderTrendsChart(labels, data, canvasId) {
      ...
      trendsInstances[canvasId] = new Chart(canvas.getContext('2d'), {
        ...
        data: {
          labels: labels,
          datasets: [{
            label: hasPermission('view_prices') ? 'ยอดการเบิก (บาท)' : 'ยอดการเบิก (🔒)',
            data: data.map(function (v) { return hasPermission('view_prices') ? v : 0; }),
            borderColor: '#8b5cf6',
    ...
    function showUserSubStock(targetId, title, isTeam) {
      ...
      API.getSubStock({ employeeId: isTeam ? null : targetId, teamId: isTeam ? targetId : null }).then(function (res) {
        ...
        body.innerHTML = '<div class="flex justify-between items-center mb-4"><h4>📦 คลังย่อย: ' + title + '</h4>' +
          ...
          '<div class="table-wrap"><table><thead><tr><th>สินค้า</th><th>ไซส์</th><th>จำนวน</th></tr></thead><tbody>' +
          items.map(function (i) {
    ...
    function downloadCSV(type) {
      ...
      if (type === 'InventoryLogs') {
        ...
        showToast('ไม่รองรับการ Export ประเภทนี้ในขณะนี้', 'warning');
        return;
      }
